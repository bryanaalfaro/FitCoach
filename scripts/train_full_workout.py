...
# All Rights Reserved.
"""Finetune Stream-VLM on full-length (≈3.5 min) QEVD Fit-Coach workouts."""
...
def build_streaming_sequence(
    ...
    loss_mask: List[int] = [0] * len(input_ids)  # ignore prompt tokens
    ...
    if response_ts.size:
        offset = response_ts[0] - video_timestamps[0]
        if np.abs(offset) < 3600:
...
def train() -> None:
    ...
    training_dataset = FullWorkoutTrainingDataset(
        hf_dataset,
        tokenizer=model.tokenizer,
        max_sequence_length=training_cfg.get("max_sequence_length"),
    )
    collator = FullWorkoutCollator(model.tokenizer.pad_token_id or model.tokenizer.eos_token_id)
